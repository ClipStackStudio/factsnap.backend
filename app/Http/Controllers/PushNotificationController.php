...
class PushNotificationController extends Controller
{
     ...
     *   description="Register or update the push notification device token for the authenticated user with platform information and activation status.",
     *   @OA\RequestBody(
     *     required=true,
     *     @OA\JsonContent(
     *       required={"push_token", "platform"},
     *       @OA\Property(property="push_token", type="string", description="Push notification device token", example="a1b2c3d4e5f6..."),
     *       @OA\Property(property="platform", type="string", enum={"ios", "android", "web"}, description="Device platform", example="ios"),
     *       @OA\Property(property="is_token_active", type="boolean", description="Whether the token should be active", example=true)
     *     )
     *   ),
     *   @OA\Response(
     *     response=200,
     ...
     *       @OA\Property(property="message", type="string", example="Push token registered successfully"),
     *       @OA\Property(property="data", type="object",
     *         @OA\Property(property="push_token", type="string", description="Registered push token"),
     *         @OA\Property(property="platform", type="string", description="Device platform"),
     *         @OA\Property(property="is_token_active", type="boolean", description="Token activation status"),
     *         @OA\Property(property="token_registered_at", type="string", format="date-time", description="Registration timestamp")
     *       )
     *     )
     *   ),
     *   @OA\Response(
    ...
    public function registerDeviceToken(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                'push_token' => ['required', 'string', 'min:10'],
                'platform' => ['required', 'string', 'in:ios,android,web'],
                'is_token_active' => ['sometimes', 'boolean'],
            ]);
        } catch (ValidationException $e) {
            return $this->validationErrorResponse($e->errors(), 'Invalid push token data.');
        }
        ...
        $updateData = [
            ...
            'is_token_active' => $validated['is_token_active'] ?? true,
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'token_registered_at' => $user->token_registered_at,
            ]
        ]);
    }
    ...
    /**
     * Get current push token information.
     * 
     * @OA\Get(
     *   path="/api/user/push-token",
     *   tags={"Push Notifications"},
     *   security={{"firebaseAuth": {}}},
     *   summary="Get current push token information",
     *   description="Get the current push notification token and platform information for the authenticated user.",
     *   @OA\Response(
     *     response=200,
     *     description="Push token information retrieved successfully",
     *     @OA\JsonContent(
     *       @OA\Property(property="success", type="boolean", example=true),
     *       @OA\Property(property="data", type="object",
     *         @OA\Property(property="push_token", type="string", description="Current push token", nullable=true),
     *         @OA\Property(property="platform", type="string", description="Device platform", nullable=true),
     *         @OA\Property(property="is_token_active", type="boolean", description="Token activation status"),
     *         @OA\Property(property="token_registered_at", type="string", format="date-time", description="Registration timestamp", nullable=true)
     *       )
     *     )
     *   )
     * )
     */
    public function getPushToken(Request $request): JsonResponse
    {
        $user = $request->user();
     ...
     *   description="Deactivate the current push notification token for the authenticated user without removing it.",
     *   @OA\Response(
     *     response=200,
     *     description="Push token deactivated successfully",
     *     @OA\JsonContent(
     *       @OA\Property(property="success", type="boolean", example=true),
     *       @OA\Property(property="message", type="string", example="Push token deactivated successfully")
     *     )
     *   )
     * )
     */
    public function deactivatePushToken(Request $request): JsonResponse
    {
        $user = $request->user();
